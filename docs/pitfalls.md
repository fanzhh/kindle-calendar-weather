...
2. **定时唤醒后 WiFi 是断的**。在休眠前关掉 WiFi、唤醒后再打开，重连才可靠。
   但**等待时间要给足**：实测唤醒后 WiFi 恢复耗时不稳定，有时 15 秒就够，
   有时要 30 秒以上。只等 20~50 秒会间歇性失败，给到 150 秒（分三段重试）才稳。

   ```
   策略1：开无线 → 等 15s → 探测
   策略2：关无线 → 等 5s → 开无线 → 等 25s → 探测
   策略3：每 15s 探测一次，最多 10 次
   ```

   实测 8 次定时唤醒全部成功（策略1 命中 5 次、策略2 命中 2 次）。
   诊断时把网卡状态也记进日志，一眼就能看出卡在哪一层：

   ```
   休眠前：无线=1 IP=192.168.x.x 默认路由=有
   关无线后：无线=0 IP=无 默认路由=无
   唤醒瞬间：无线=0 IP=无 默认路由=无
   策略1后：无线=1 IP=192.168.x.x 默认路由=有   ← 恢复了
   ```
